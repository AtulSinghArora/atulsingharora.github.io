...
\begin{longtable}{r|p{11cm}}
  \textsc{2021-}\emph{present}
                   ...
                   &\small Advisor: Prof. Thomas \textsc{Vidick}\\
                    &\footnotesize{Primarily studied hybrid models where depth bounded quantum circuits, $\QNC$, can be interleaved with $\BPP$ machines.}\\
                    &\footnotesize{~~ End of 2021: Showed that seemingly similar ways of composing classical and bounded depth quantum circuits result in models with incomparable power: $\QC \nsubseteq \CQ$ and $\CQ \nsubseteq \QC$ relative to oracles. The second separation gives evidence that adaptive measurements can be quite powerful. This was shown relative to a non-standard oracle which we called a ``stochastic oracle''.\su{1}}\\
                    % &\footnotesize{~~ End of 2022: Showed oracle separations of bounded-depth hybrid quantum-classical circuits ($\mathsf{BPP}^\mathsf{QNC} \nsubseteq \mathsf{QNC}^\mathsf{BPP}$ and $\mathsf{QNC}^\mathsf{BPP} \nsubseteq \mathsf{BPP}^\mathsf{QNC}$) and recovered previous results showing $\CQ \cup \QC \subsetneq \BQP$ (original proofs had errors). One of the separations is relative to what we call a ``stochastic'' oracle.\su{1}}\\
                    &\footnotesize{~~End of 2022: Relative to a random oracle, gave the following characterisation of quantum depth.\su{2} %(Separations relative to random oracles can be heuristically instantiated in practice, using cryptographic hash functions.)
                    \begin{itemize}
                      \item Depth bounded hybrid quantum-classical circuits are strictly weaker than (poly depth) quantum circuits ($\mathsf{BPP}^{\mathsf{QNC}^\mathsf{BPP}} \neq \mathsf{BQP}$). %This refutes Jozsa's conjecture in the random oracle model and resolves one of Aaronson's ten semi-grand challenges in quantum computing. 
                      \item The separations among the hybrid classes above, now relative to a random oracle.
                      \item There is a two-message proof of quantum depth protocol which can be instantiated using the recent proof of quantumness protocol by Yamakawa and Zhandry. %In particular, the protocol is insensitive to classical depth.
                    \end{itemize}} \\
                    ...
                    \begin{itemize}
                      ...
                      \item Collected all our previous results on the topic into a journal version---Solutions to Quantum Weak Coin Flipping.\su{5}
                    \end{itemize}
                    } \\
